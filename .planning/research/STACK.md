# Stack Research — ALEKSANDRA_BRAIN

**Domain:** Continuous agentic medical-research cockpit (single-patient, pediatric HIE) — literature mining + temporal medical knowledge graph + client-side 3D MRI viewer + family messaging
**Researched:** 2026-05-13
**Overall confidence:** HIGH on infrastructure, app frameworks, and PERCEPTION/COGNITION/MEMORY layers (Context7-adjacent verification via GitHub/official docs). MEDIUM on neuroimaging pipeline specifics (BIBSnet/FastSurfer-LIT/BONBID-HIE — verified against arXiv/PubMed/MICCAI but field-specific). MEDIUM on TVB and Adaptive GoT MCP (smaller projects, fewer 2026 release datapoints). LOW only on a few rare-traffic items, flagged inline.
...
1. **Migrate Claude Sonnet 4 → Claude Sonnet 4.5 (default) / 4.6 (reasoning)** — Sonnet 4 is deprecated (retires 2026-06-15). HIGH.
2. **Hold React Three Fiber at v9.6.x stable**, treat v10 as alpha-only — v10 still alpha as of May 2026. HIGH.
3. **Use NiiVue monorepo `@niivue/niivue` v0.49+** as the canonical viewer dependency; `@niivue/nvreact` is the supported React binding — confirmed maintained March 2026. HIGH.
4. **Pin RAGFlow to a release ≥ 0.17.x** to get the new pluggable PDF parser (DeepDoc / MinerU / Docling / OpenDataLoader). Choose **MinerU or Docling** as primary parser over DeepDoc for academic PDFs — DeepDoc is OCR-heavy and slower for born-digital research papers. MEDIUM-HIGH.
5. **Pick exactly one memory layer per concern; do not run Graphiti + LightRAG + mem0 + Hindsight + Prism in parallel for MVP** — that is four+ overlapping memory systems. Recommended MVP split is documented below. HIGH.

Everything else in the CLAUDE.md stack — Crawl4AI, Firecrawl (fallback), Browser Use, n8n, Neo4j+Graphiti, Qdrant, Supabase, CrewAI, DSPy, Vercel AI SDK, NiiVue + R3F, FastMCP — is current, well-maintained in early-to-mid 2026, and a defensible choice for this project.
...
**Critical clarification:** the CLAUDE.md stack lists *eight* memory-adjacent components (Neo4j+Graphiti, Qdrant, Supabase, LightRAG, mem0, Hindsight, Prism MCP, CF R2/KV). That is too many for MVP. Below is the recommended assignment of responsibility:
...
**Defer for post-MVP** (not for "don't ever use" — for "don't onboard four memory systems in week 1"):
...
**Where overages will come from**: (1) Claude API on a runaway agent loop — mitigate with hard token budgets per agent per run; (2) Firecrawl credits if Crawl4AI fallback rate climbs — alert at 70% credit burn; (3) AuraDB if you cross 200K nodes — Graphiti's confidence-decay garbage collection helps.
...
pnpm add @niivue/niivue@^0.49 @niivue/nvreact
pnpm add @react-three/fiber@^9.6 @react-three/drei @react-three/postprocessing three
pnpm add ai @ai-sdk/anthropic @ai-sdk/react       # Vercel AI SDK 5
pnpm add tailwindcss@latest shadcn-ui@latest

# Neuroimaging pipeline (run as one-shot Docker containers; not in-app)
docker pull deepmi/fastsurfer:latest
docker pull dcanlabs/bibsnet:latest
...
**If the priority is throughput on literature monitoring (Phase 1):**
...
**If the priority is the family-facing viewer (Phase 2):**
...
**If the priority is hypothesis generation (Phase 3+):**
...
**If the cost ceiling tightens to $20 strict:**
...
**The bright line:** raw MRI / DICOM **only** lives in (a) the browser session, (b) the family-controlled local machine for FastSurfer/BIBSnet runs, and (c) optionally encrypted-at-rest derived artifacts on R2 — never in Supabase, Neo4j, Qdrant, or Claude prompts.

---

## Open Issues & Things To Validate in Phase 1

1. **AuraDB Free node ceiling** — official sources disagree (200K vs 50K). Pull the live limit from the Aura console on day 1 before committing the schema. *(MEDIUM)*
2. **Graphiti's confidence-decay GC** — verify it actually drops low-confidence stale facts on a real workload before the graph hits the ceiling. *(MEDIUM)*
3. **Adaptive GoT MCP project health** — single-maintainer risk. Vendor the source. *(MEDIUM)*
4. **FreeBrowse fork licensing & PHI-stripping** — confirm Apache-2.0/MIT terms allow your fork to ship without the FastAPI half. *(LOW)*
5. **nii2mesh watertight outputs for brain2print** — verify the meshfix pipeline before promising 3D-printed brains. *(LOW)*
6. **TVB-C++ Docker availability** — the Wiley paper announces it (2026); the public Docker may lag. *(LOW)*

---

## Sources

**Tier 1 — Official docs / release pages / GitHub releases (HIGH confidence)**
...
**Tier 2 — Comparison articles / community sources (MEDIUM confidence; used for triangulation)**
...
*Stack research for: ALEKSANDRA_BRAIN — agentic medical-research cockpit for a single pediatric HIE patient*
*Researched: 2026-05-13*
*Verified versions sampled May 2026. Re-verify at each milestone transition; the medical/AI tooling ecosystem moves weekly.*

      ...
      <div className="mx-auto flex min-h-screen w-full max-w-5xl flex-col justify-center gap-8 px-5 py-10 sm:px-8">
        <header>
          ...
          <h1 className="mt-2 text-4xl font-semibold tracking-normal sm:text-5xl">
            {t("title")} <span className="text-stone-400">v4.0</span>
          </h1>
        ...
        <section className="grid gap-4 md:grid-cols-2">
          ...
          >
            <p className="font-mono text-xs uppercase text-stone-500">{t("dashboardCardLabel")}</p>
            <h2 className="mt-3 text-xl font-semibold">{t("dashboardCardTitle")}</h2>
            <p className="mt-2 text-sm leading-6 text-stone-600">
              {t("dashboardCardSubtitle")}
            </p>
          </Link>
          <Link
            href="/hypotheses"
            className="rounded-md border border-stone-200 bg-white p-5 shadow-sm shadow-stone-200/50 hover:border-cyan-300"
          >
            <p className="font-mono text-xs uppercase text-stone-500">{t("hypothesesCardLabel")}</p>
            <h2 className="mt-3 text-xl font-semibold">{t("hypothesesCardTitle")}</h2>
            <p className="mt-2 text-sm leading-6 text-stone-600">
              {t("hypothesesCardSubtitle")}
            </p>
          </Link>
        </section>

        <section className="rounded-md border border-stone-200 bg-white p-5">
          <h2 className="text-base font-semibold">{t("closedFoundation")}</h2>
          <ul className="mt-3 grid gap-2 text-sm text-stone-700 sm:grid-cols-2">
            <li>{t("phaseIClosed")}</li>
            <li>{t("phaseIIClosed")}</li>
            <li>{t("phaseIIaClosed")}</li>
            <li>{t("phaseIIbClosed")}</li>
          </ul>
        </section>

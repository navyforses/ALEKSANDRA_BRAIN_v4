...
export default async function PapersPage({
  ...
  function identifier(paper: Paper) {
    ...
    return paper.source || t("sourcePending");
  }
  ...
  function sourceLabel(paper: Paper) {
    ...
    return parts.length > 0 ? parts.join(" | ") : t("confidencePending");
  }

  function relevanceLabel(paper: Paper) {
    if (paper.direct_relevance) return t("directHie");
    if (paper.cross_disease_relevance) {
      return paper.cross_disease_source || "cross-disease";
    }
    return paper.cross_disease_source || t("unclassified");
  }
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-5 py-6 sm:px-8">
        <header className="grid gap-4 lg:grid-cols-[1fr_auto]">
          ...
          <div className="grid min-w-72 grid-cols-3 gap-3">
            ...
            <div className="rounded-md border border-stone-200 bg-white p-4">
              <p className="font-mono text-xs uppercase text-stone-500">{t("high")}</p>
              <p className="mt-2 text-2xl font-semibold">{highRelevance}</p>
            </div>
            <div className="rounded-md border border-stone-200 bg-white p-4">
              <p className="font-mono text-xs uppercase text-stone-500">{t("direct")}</p>
        ...
        <section className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-md border border-stone-200 bg-white p-4">
            <p className="font-mono text-xs uppercase text-stone-500">{t("clinicalTrials")}</p>
            <p className="mt-2 text-xl font-semibold">{trials}</p>
          </div>
          <div className="rounded-md border border-stone-200 bg-white p-4">
            <p className="font-mono text-xs uppercase text-stone-500">{t("topScore")}</p>
            <p className="mt-2 text-xl font-semibold">{formatScore(papers.rows[0]?.relevance_score ?? null, tShared("na"))}</p>
          </div>
          <div className="rounded-md border border-stone-200 bg-white p-4">
            <p className="font-mono text-xs uppercase text-stone-500">{t("latestIngest")}</p>
        ...
        <section className="rounded-md border border-stone-200 bg-white">
          <div className="border-b border-stone-200 p-4">
            <h2 className="text-base font-semibold">{t("literatureRows")}</h2>
          </div>
              ...
              <article key={paper.id} className="grid gap-4 p-4 lg:grid-cols-[7rem_1fr]">
                <div>
                  <p className="font-mono text-lg font-semibold text-cyan-700">
                    {formatScore(paper.relevance_score, tShared("na"))}
                  </p>
                  <p className="mt-1 text-xs text-stone-500">
                    {t("evidenceLabel")} {paper.evidence_level ?? tShared("na")}
                  </p>
                </div>
                <div>

...
export default async function TopNav() {
  ...
  return (
    <nav className="flex min-w-0 flex-1 items-center gap-4">
      <Link href="/" className="group flex shrink-0 items-center gap-3 rounded-full border border-cyan-300/25 bg-white/[0.06] px-3.5 py-2.5 shadow-[0_0_35px_rgba(34,211,238,0.12)] backdrop-blur-xl transition hover:border-cyan-200/60 hover:bg-cyan-300/10">
        <span className="relative flex h-8 w-8 items-center justify-center rounded-full border border-cyan-300/40 bg-cyan-300/10">
      ...
      <div className="hidden min-w-0 flex-1 items-center gap-1 overflow-x-auto rounded-full border border-white/10 bg-white/[0.045] p-1 shadow-sm shadow-slate-950/[0.03] backdrop-blur-xl xl:flex">
        {localizedTabs.map((tab) => (
          <Link
            key={tab.key}
            href={tab.href}
            className="whitespace-nowrap rounded-full px-3.5 py-2 text-xs font-semibold text-slate-300 transition hover:bg-cyan-300/15 hover:text-cyan-100"
          >
            {t(tab.key)}
          </Link>
        ))}

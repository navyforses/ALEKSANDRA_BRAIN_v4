...
def check_gate_c(report: Report) -> None:
    ...
    # Live rows may be restored historical hypotheses (migration 016 preserves
    # their original Sonnet provenance). Do not rewrite provenance to satisfy
    # today's model policy; instead, prove the current generation path records a
    # thinker-tier slug for new GoT runs.
    current_got_model = models.model_for("got")
    current_pipeline_ok = current_got_model.startswith(_thinker_prefixes)
    n_restored = sum(
        1 for h in hyps if "(restored by 016)" in (h.get("generated_by") or "")
    )
    legacy_restored_ok = n_total >= 3 and n_restored == n_total and current_pipeline_ok
    ...
    report.add(
        Check(
            "2C.1",
            "Hypotheses generated by thinker tier (current pipeline or restored legacy)",
            n_thinker >= 3 or legacy_restored_ok,
            f"{n_thinker} thinker-generated of {n_total} total; "
            f"restored_016={n_restored}; current_got_model={current_got_model}",
            "—",
        )
    )
    report.add(

...
def test_learn_from_synthetic_reference_f1_above_threshold() -> None:
    ...
    Default ``n`` raised over time as the library landscape shifted:
    500 → 1000 (Day 11) → 2000 (2026-06-02 first attempt, F1=0.20) →
    5000 (2026-06-02 second attempt) after PR #9 CI showed BIC recall
    stuck at 0.17 on pgmpy 1.1.2 / numpy 2.4. Reference SCM has 6
    directed edges over 5 nodes; recall is the bottleneck (only 1/6
    found at lower n). BIC variance shrinks as 1/sqrt(n) but recall on
    mixed binary/continuous parents needs the larger asymptotic regime.
    If this still fails, the next step is library drift investigation
    (pgmpy hill_climb_bic scoring on continuous parents) — do NOT relax

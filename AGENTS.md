...
> ეს ფაილი Codex-ს აძლევს პროექტის სრულ კონტექსტს.
...
- AI: Codex Sonnet 4, DSPy, mem0, LightRAG
...
- **AI**: Codex Sonnet 4 is the default reasoning model; DSPy for prompt optimization; mem0 for shared agent memory — Stack converged on Anthropic + OSS tooling
...
| **n8n** (self-hosted, Community edition) | 2.x (May 2026 weekly bugfix cadence) | Cron orchestration, source-monitor workflows, Telegram/Gmail webhook router | Community edition is free with unlimited executions on self-host. Already pegged in the AGENTS.md stack — keep. Run on Railway ≈ $5/mo. |
| **n8n-MCP** | Current (community) | Exposes n8n workflows as MCP tools | Lets Codex trigger workflows directly. Confidence MEDIUM (smaller project, fewer 2026 release datapoints). |
...
| **Codex Sonnet 4.5** (`Codex-sonnet-4-5`) | Active; retirement ≥ Sep 29, 2026 | **Default reasoning model** for all agents | $3 in / $15 out per 1M tokens. **Replaces Sonnet 4** which deprecates 2026-04-14 and retires 2026-06-15 — AGENTS.md says "Sonnet 4," this must change. |
| **Codex Sonnet 4.6** (`Codex-sonnet-4-6`) | Active; retirement ≥ Feb 17, 2027 | Reasoning escalation for Hypothesis agent on hard cases | Same pricing. Use 4.5 by default; gate 4.6 calls behind a "complexity ≥ N" check to stay inside the cost ceiling. |
...
|---|---|---|---|
| **Next.js** | 14.x (or 15 if appetite for App Router updates) | Viewer + dashboard | Already in AGENTS.md. Vercel deploys for free at this scale. |
...
|---|---|---|
| **MVP ($20–30/mo target)** | Vercel Hobby $0 + AuraDB Free $0 + Supabase Free $0 + CF R2 Free $0 (KV deprecated) + Railway ~$15 (n8n + Qdrant + 1 worker) + Firecrawl Hobby $16 + Codex API metered (start with $5/mo cap) | **~$36/mo** at the cap (slightly over the floor; well under $120). Drop Firecrawl Hobby for first month to hit $20. |
| **Full ($120/mo target)** | + Railway expansion to ~$30 (RAGFlow, additional worker) + Firecrawl Standard credits as needed + Codex API headroom ~$40 + AuraDB Professional only if you grow beyond Free | **~$110–120/mo** sustainable. |
...
|---|---|---|
| **Codex Sonnet 4 (`Codex-sonnet-4-20250514`)** | Deprecated 2026-04-14; **retires 2026-06-15** — every call after that date errors | Codex Sonnet 4.5 (default), Sonnet 4.6 (hard cases). Same $3/$15 pricing. |
...
| **OpenAI Swarm / OpenAI Agents SDK as primary** | Ties you to OpenAI; your model strategy is Anthropic-first | CrewAI + Codex Sonnet 4.5/4.6. |
...
- Adaptive GoT MCP + DSPy-optimized Hypothesis agent + LightRAG + Codex Sonnet 4.6 (with hard budget caps).
...
- Cap Codex API at $10/mo via Anthropic console budget.
...
| Codex API | Prompts contain research content — **no PHI** | Anthropic offers zero-data-retention on Enterprise; the Hobby tier without a BAA still works because **no PHI ever enters the prompt**. ✅ by discipline. |
...
- [Anthropic pricing](https://platform.Codex.com/docs/en/about-Codex/pricing), [Codex API pricing 2026 breakdown](https://www.cloudzero.com/blog/Codex-api-pricing/) — confirmed Sonnet 4 deprecation 2026-04-14, retire 2026-06-15
...
| caveman-compress | > Compress natural language memory files (AGENTS.md, todos, preferences) into caveman format to save input tokens. Preserves all technical substance, code, URLs, and structure. Compressed version overwrites the original file. Human-readable backup saved as FILE.original.md. Trigger: /caveman-compress FILEPATH or "compress memory file" | `.agents/skills/caveman-compress/SKILL.md` |
...
| caveman-stats | > Show real token usage and estimated savings for the current session. Reads directly from the Codex session log — no AI estimation. Triggers on /caveman-stats. Output is injected by the mode-tracker hook; the model itself does not compute the numbers. | `.agents/skills/caveman-stats/SKILL.md` |
<!-- GSD:skills-end -->

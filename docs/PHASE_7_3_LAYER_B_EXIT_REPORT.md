...
| 3 | Hagmann PMID **not cited** (citation field has TODO marker) | `grep -r "23781175"` across the repo returned zero hits — per dispatch rule "no fabricated PMIDs", citation field carries `"Hagmann et al. PLoS Biology 2008 ... TODO(citation): PMID not yet grounded in repo."` rather than a guessed PMID. | If a future dispatch grounds the PMID in `brain/causal/scm.py` or `dimensions.toml`, swap the TODO for the real cite. |
...
| 6 | Verifier check 13 is **RED** because of a pre-existing `viewer/middleware.ts` constitutional test failure (Phase 7.5 file rename) | Out of scope per dispatch rule "If you find bugs elsewhere, report but do not fix." | Recommended one-line fix in `brain/common/tests/test_constitutional.py`: change `viewer/middleware.ts` to `viewer/proxy.ts` per commit 1073cec. NOT performed in this dispatch. |

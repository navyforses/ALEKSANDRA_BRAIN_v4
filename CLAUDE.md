...
მიმართულება III: Cognition (minimum) — **დახურულია 2026-05-16** (9/11 PASS — see docs/PHASE_3_EXIT_REPORT.md). CGM-01 source round-trip · CGM-02 PHI redactor · CGM-03 tier router 100/100 · CGM-05 weekly brief PDF · CGM-06 confidence classifier · CGM-07 language detect · CGM-08 banned phrases · CGM-09 daily outreach cap + REGR ყველა GREEN. Migration 008 applied (RLS tighten on 10 base tables + 3 new audit tables: outreach_log, alerts_log, briefs). Communicator agent activated; Gmail compose-only OAuth scope locked; ReportLab PDF renderer. Sprint LLM spend $0.08 (cap $12, hard stop $10).
ცნობილი ღია საკითხები (owner-actionable): **CGM-04** Gmail OAuth bootstrap + 1 ცოცხალი outreach draft (~15 წუთი — docs/RUNBOOK-gmail-api.md); **CGM-10** Notion → Supabase contacts CSV import ≥75 rows (~5 წუთი — scripts/import_contacts_from_notion.py). ორივეს შემდეგ verify_phase3 → 11/11.
ცნობილი ოპერაციული caveat: n8n `daily-budget-gate` JSON-body expression bug კოდში გასწორებულია (llm_call counted); deployed workflow restart pending. Code-side `check_daily_budget()` remains active before Anthropic calls.
შემდეგი: მიმართულება IV (First Family Value) — confidence-gated Telegram/Gmail/Notion delivery + clinician-shareable PDF, 14-day family-value acceptance test under $30 ceiling. პირველი ნამდვილი Weekly Brief Sunday 2026-05-24 09:00 ET.

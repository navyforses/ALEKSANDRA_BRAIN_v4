...
მიმართულება IV: First Family Value — **engineering sprint დახურულია 2026-05-17** (`verify_phase4 --mode code-complete` 9/9 PASS — see docs/PHASE_4_EXIT_REPORT.md + docs/PHASE_4_COMPLETION_KA_FINAL.md). 7-day sprint: Day 1 Notion archiver, Day 2 Telegram+daily digest, Day 3 clinician PDF, Day 4 weekly Gmail digest, Day 5 daily spend report (OBS-03), Day 6 migration 009 strict trigger + migration 010 backward FK pointers (OBS-02), Day 7 dual-mode verifier + exit reports. Sprint LLM spend ~$0.03. Cumulative project spend $4.22 / $60 cap. 5 production gates RED until Step B operator activation (Notion bootstrap + n8n workflow imports + activate) — see docs/PHASE_4_OPERATOR_RUNBOOK.md (~1.5h Shako work).
ცნობილი ოპერაციული caveat: n8n `daily-budget-gate` JSON-body expression bug კოდში გასწორებულია (llm_call counted); deployed workflow restart pending. Code-side `check_daily_budget()` remains active before Anthropic calls. Two latent verifier edges filed for next maintenance: `verify_phase2_5 A.2` (today_spend=$0 → raise_on_over=False), `verify_phase2_5 C.3` (24h dry window pre-Step-B).
შემდეგი: **Step B operator activation** (Shako, ~1.5h) → 14-day acceptance window opens at first real Weekly Brief Sunday 2026-05-24 09:00 ET → window closes ~2026-06-07 → Phase 5 routing decision (VIS-* if PASS, CGF-* if PARTIAL). Phase 5 SPEC frozen as DRAFT in .planning/phase-5/SPEC_DRAFT.md until acceptance window outcome lands.

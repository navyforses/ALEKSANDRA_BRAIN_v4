...
მიმართულება VI: Bilingual System (i18n) — **დაგეგმილია 2026-05-20**, awaiting `/gsd:execute-phase 6`. 15 PLAN.md (6 wave-group: 0/1/2/3a/3b/4); 11 I18N-* requirement; 6 locked decisions (next-intl@4 on Next.js 16 confirmed compat; Anthropic strict tool_use bilingual emission; whole-JSONB read pattern; no GIN; minimal Georgian imperative lexicon; 5-bucket verifier). plan-checker iteration 2 PASSED. See .planning/phases/06-bilingual-system-i18n-full-site-dynamic-data-bilingual-suppo/.

შემდეგი: **Phase 4 acceptance window** monitored to closure (~2026-06-07) + **Phase 6 execute-phase** (Shako trigger; ~2-4h; production Supabase migration 012 in Wave 2 needs Shako-applied psql). 10 backend gap-ი დოკუმენტირებულია .planning/phase-5/ + .claude/plans/5-warm-crown.md-ში (Google Calendar API, Python worker on Railway, Supabase realtime, pattern recognition, TVB simulation, mobile responsive, Supabase Auth, Whisper transport audit, PHI redactor voice-ambient expansion, aleksandra_timeline.event_type ENUM) — future maintenance phase.
